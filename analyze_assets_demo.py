#!/usr/bin/env python3
"""
Asset Return Analysis Tool (Demo with sample data)
各アセット（株式、債券、金、原油）の年別・月別騰落率を分析

NOTE: This is a demo version using sample data.
For live data, you need Python 3.10+ to use yfinance.
"""
...
class AssetAnalyzer:
    ...
    def generate_sample_data(self):
        ...
        for ticker in self.tickers:
            if ticker == 'SPY':  # 米国株 - 長期的に上昇トレンド
                returns = np.random.normal(0.0003, 0.01, len(dates))  # 平均0.03%/日
                price = 100 * np.exp(np.cumsum(returns))
            elif ticker == 'TLT':  # 債券 - 低リターン、低ボラティリティ
                returns = np.random.normal(0.0001, 0.005, len(dates))
                price = 100 * np.exp(np.cumsum(returns))
            elif ticker == 'GLD':  # 金 - 中程度のボラティリティ
                returns = np.random.normal(0.0002, 0.008, len(dates))
                price = 100 * np.exp(np.cumsum(returns))
            else:  # USO - 原油 - 高ボラティリティ
                returns = np.random.normal(0.0, 0.015, len(dates))
                price = 100 * np.exp(np.cumsum(returns))

            self.data[ticker] = pd.Series(price, index=dates)
            print(f"  {ticker} ({ASSETS[ticker]}): {len(dates)}日分")
    ...
    def create_annual_returns_table(self):
        ...
        ax.axis('off')
        ax.set_title('年別リターン（%） - サンプルデータ', fontsize=16, fontweight='bold', pad=20)
    ...
    def create_heatmap(self):
        ...
        ax.set_title('年別リターン ヒートマップ - サンプルデータ', fontsize=16, fontweight='bold')
    ...
    def create_cumulative_returns_plot(self):
        ...
        ax.set_title('資産クラス別 累積リターンの推移 - サンプルデータ', fontsize=16, fontweight='bold')
    ...
    def create_annual_bar_chart(self):
        ...
        ax.set_title('資産クラス別 年次リターン - サンプルデータ', fontsize=16, fontweight='bold')
        ax.set_xticks(x)
    ...
    def create_monthly_heatmap(self):
        ...
        for ticker in self.monthly_returns.columns:
            ...
            ax.set_ylabel('年', fontsize=12)
            ax.set_title(f'{ticker} ({ASSETS[ticker]}) - 月別リターン (サンプルデータ)',
    ...
    def run_analysis(self):
        ...
        print("="*80)
        print("\nNOTE: これはデモ版です。")
        print("実際のデータを使用するには Python 3.10+ が必要です。")
